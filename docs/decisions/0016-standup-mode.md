# 0016. Standup mode: a rolling look-back/look-forward report, starring, and per-card exclusion
...
standup, plus a way to keep the report from being cluttered by things not worth mentioning, and a
...
- **Both are free-text inputs, not a dropdown or `<input type=number>`**, clamped server-side by
  the pure `standup.clamp_days()`. 0 is a real, meaningful value ("show nothing that direction"),
...
- **Exclusion is per-card with two scopes, stored server-side** (`.trellis.yml`'s
  `standup_exclusions`, shared across the LAN instance like saved searches): "today"
  (`until=<today's ISO date>`, expires automatically — `standup.excluded_ids()` only honors it on
...
- **Starring is a persistent per-card boolean (`Card.starred`), not standup-scoped.** It's exposed
  everywhere the card-edit dialog opens (`_edit.html`'s `.star-btn`, shared via one `ui.js` click
  handler keyed off the class, not the template) as well as from a Standup row directly — matching
  the "duplicate from anywhere" precedent (ADR 0014). A standup-only flag would have meant
...
- **Two independent show/hide mechanisms, matching two different existing patterns, not one:**
  - The on-page filter popover (back/forward/starred-only, `.standup-filter-wrap` — its own class
    trio per AGENTS.md trap 9) also carries "Completed"/"Upcoming" section-visibility checkboxes,
    stored in `localStorage` (`standup-hide`) via a `StandupSections` module mirroring `TodaySections`.
  ...
  - Settings gained a "Standup" checkbox in the existing "Show in the sidebar" and "Page titles"
    fieldsets (same as every other page), plus a new "Excluded from Standup" section listing every
    resolved exclusion (title, board, "today only" vs "always") with an "Include again" button —
    the one piece of standup state that's a genuine standing, shared setting (unlike section
...
- **A dedicated `_standup_row.html` macro, not an extension of `_card_row.html`/`_logbook_row.html`.**
  Standup's row needs star + exclude-menu + either a completed-stamp or a due date depending on
  ...
  standup-only macro keeps the blast radius contained.
...
- Unit tests cover `standup.py`'s pure functions (`excluded_ids`, `clamp_days` — zero, negative,
  decimal, huge, and whitespace-padded input) and the new `Store` methods (`set_starred`,
  `exclude_from_standup`/`include_in_standup` round-trips, the missing-card `KeyError` path).
  Route tests cover `/standup`'s default window, `back=0`/`forward=0`, `starred=1`, and both
  exclusion scopes including "today"'s same-day expiry. Two e2e tests cover starring from the edit
  dialog and seeing it reflected on a Standup row, the starred-only filter actually narrowing
